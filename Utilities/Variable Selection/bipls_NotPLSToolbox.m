function biModel=bipls_NotPLSToolbox(X,Y,no_of_lv,prepro_method,intervals,xaxislabels,val_method,segments)
...
%
%  biModel=bipls_NotPLSToolbox(X,Y,no_of_lv,prepro_method,intervals,xaxislabels,val_method,segments);

%  Functions used: iPLSreverse

if nargin==0
    disp(' ')
    disp(' biModel=bipls_NotPLSToolbox(X,Y,no_of_lv,prepro_method,intervals,xaxislabels,val_method,segments);')
    disp(' ')
    disp(' Example:')
    disp(' biModel=bipls_NotPLSToolbox(X,Y,10,''mean'',20,[],''syst123'',5);')
    disp(' ')
    return
end
...
ModelReverse = bipls_sub_iPLSreverse(X,Y,no_of_lv,prepro_method,intervals,xaxislabels,val_method,segments);
...
SizeInt = allint(1:end-1,3)+1 - allint(1:end-1,2);

for i=2:(intervals-1)
    maxSizeInt = max(SizeInt(keeptrackofinterval(:,2)));
    
    %plot(newX'),pause
    RevVars(i)=size(newX,2);
    if RevVars(end)-maxSizeInt >= no_of_lv
        ModelReverse = bipls_sub_iPLSreverse(newX,Y,no_of_lv,prepro_method,intervals-(i-1),xaxislabels,val_method,segments);

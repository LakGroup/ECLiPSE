function ModelsReverse = bipls_sub_iPLSreverse(X,Y,no_of_lv,prepro_method,intervals,xaxislabels,val_method,segments)
...
%
%  ModelsReverse=sub_iPLSreverse(X,Y,no_of_lv,prepro_method,intervals,xaxislabels,val_method,segments);

%  Function used: sub_pls_val

if nargin==0
   disp(' ')
   disp(' ModelsReverse=sub_iPLSreverse(X,Y,no_of_lv,prepro_method,intervals,xaxislabels,val_method,segments);')
   disp(' ')
   disp(' Example:')
   disp(' ModelsReverse=sub_iPLSreverse(X,Y,7,''mean'',20,[],''syst123'',5);')
   disp(' ')
   return
end
...
%    end
   ...
   if i==ModelsReverse.intervals+1
       ModelsReverse.PLSmodel{i} = bipls_sub_pls_val(X(:,[ModelsReverse.allint(i,2):ModelsReverse.allint(i,3)]),Y,no_of_lv,prepro_method,val_method,segments);
   else
       ModelsReverse.PLSmodel{i} = bipls_sub_pls_val(X(:,GlobalWithoutIntervalWithout),Y,no_of_lv,prepro_method,val_method,segments);
   end
end
